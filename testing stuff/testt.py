import cv2
import cupy as np  # Changed from numpy to cupy
...
##########################
wCam, hCam = 1280, 720  # For HD resolution
...
cv2.namedWindow("HandTracking", cv2.WND_PROP_TOPMOST)
cv2.setWindowProperty("HandTracking", cv2.WND_PROP_TOPMOST, 1)
    ...
    if len(lmList) != 0:
        ...
        if fingers[1] == 1 and fingers[2] == 0:
            ...
            if frameR < x1 < wCam - frameR and frameR < y1 < hCam - frameR:
                x3 = np.interp(np.array([x1]), np.array([frameR, wCam - frameR]), np.array([0, wScr]))[0]
                y3 = np.interp(np.array([y1]), np.array([frameR, hCam - frameR]), np.array([0, hScr]))[0]
        ...
        if fingers[0] == 0 and fingers[1] == 1 and fingers[2] == 1 and fingers[3] == 0 and fingers[4] == 0:
            ...
            # 10 click mouse if distance is as per our need
            if length < 35:
...
for bar, time in zip(bars, times):
    plt.text(bar.get_width(), bar.get_y() + bar.get_height() / 2, f'{time:.2f}s', ha='left', va='center', color='black')
